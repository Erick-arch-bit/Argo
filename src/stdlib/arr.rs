...
pub fn crear_modulo() -> Objeto {
    ...
    fn arr_map(args: Vec<Objeto>) -> Objeto {
        if args.len() != 2 {
            return Objeto::Error("arr.map: se esperaban 2 argumentos (arreglo, función)".to_string(), Vec::new());
        }
        ...
        match arr {
            Objeto::Arreglo(v) => {
                ...
                for elem in v {
                    ...
                    if let Objeto::Error(_, _) = &res { return res }
                    resultado.push(res);
                }
                Objeto::Arreglo(resultado)
            }
            _ => Objeto::Error("arr.map: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }

    fn arr_filter(args: Vec<Objeto>) -> Objeto {
        if args.len() != 2 {
            return Objeto::Error("arr.filter: se esperaban 2 argumentos (arreglo, función)".to_string(), Vec::new());
        }
        let (arr, closure) = (&args[0], &args[1]);
        match arr {
            Objeto::Arreglo(v) => {
            ...
            _ => Objeto::Error("arr.filter: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }

    fn arr_reduce(args: Vec<Objeto>) -> Objeto {
        if args.len() < 2 || args.len() > 3 {
            return Objeto::Error("arr.reduce: se esperaban 2-3 argumentos (arreglo, función, inicial?)".to_string(), Vec::new());
        }
        ...
        match arreglo {
            ...
            _ => Objeto::Error("arr.reduce: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }

    fn arr_find(args: Vec<Objeto>) -> Objeto {
        if args.len() != 2 {
            return Objeto::Error("arr.find: se esperaban 2 argumentos (arreglo, función)".to_string(), Vec::new());
        }
        let (arr, closure) = (&args[0], &args[1]);
        match arr {
            Objeto::Arreglo(v) => {
            ...
            _ => Objeto::Error("arr.find: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }

    fn arr_every(args: Vec<Objeto>) -> Objeto {
        if args.len() != 2 {
            return Objeto::Error("arr.every: se esperaban 2 argumentos (arreglo, función)".to_string(), Vec::new());
        }
        let (arr, closure) = (&args[0], &args[1]);
        match arr {
            Objeto::Arreglo(v) => {
            ...
            _ => Objeto::Error("arr.every: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }

    fn arr_some(args: Vec<Objeto>) -> Objeto {
        if args.len() != 2 {
            return Objeto::Error("arr.some: se esperaban 2 argumentos (arreglo, función)".to_string(), Vec::new());
        }
        let (arr, closure) = (&args[0], &args[1]);
        match arr {
            Objeto::Arreglo(v) => {
            ...
            _ => Objeto::Error("arr.some: el primer argumento debe ser un arreglo".to_string(), Vec::new()),
        }
    }
